...
func NewContainer(ctx context.Context, conf *config.JSON) (c *Container, err error) {
	c = &Container{
		BaseCotainer: container.NewBaseCotainer(),
		ctx:          ctx,
	}
	c.SetConfig(conf)
...
func (c *Container) Start() (err error) {
	...
	if err = c.preHandle(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v preHandle failed. err: %v", c.jobID, err))
		return
	}

	// log.Infof("DataX jobContainer %v starts to init.", c.jobID)
	if err = c.init(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v init failed. err: %v", c.jobID, err))
		return
	}
	// log.Infof("DataX jobContainer %v starts to prepare.", c.jobID)
	if err = c.prepare(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v prepare failed. err: %v", c.jobID, err))
		return
	}
	// slog.Infof("DataX jobContainer %v starts to split. err: %v", c.jobID)
	if err = c.split(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v split failed. err: %v", c.jobID, err))
		return
	}
	// slog.Infof("DataX jobContainer %v starts to schedule. err: %v", c.jobID)
	if err = c.schedule(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v schedule failed. err: %v", c.jobID, err))
		return
	}
	// slog.Infof("DataX jobContainer %v starts to post.", c.jobID)
	if err = c.post(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v post failed. err: %v", c.jobID, err))
		return
	}
	// slog.Debugf("DataX jobContainer %v starts to postHandle.", c.jobID)
	if err = c.postHandle(); err != nil {
		slog.Error(fmt.Sprintf("DataX jobContainer %v postHandle failed. err: %v", c.jobID, err))
		return
	}
...
func (c *Container) destroy() (err error) {
	if c.jobReader != nil {
		if rerr := c.jobReader.Destroy(c.ctx); rerr != nil {
			slog.Error(fmt.Sprintf("DataX jobContainer %v jobReader %s destroy error: %v",
	...
	if c.jobWriter != nil {
		if werr := c.jobWriter.Destroy(c.ctx); werr != nil {
			slog.Error(fmt.Sprintf("DataX jobContainer %v jobWriter %s destroy error: %v",
...
func (c *Container) init() (err error) {
	...
	slog.Info(fmt.Sprintf("DataX jobContainer %v reader %v inited", c.jobID, c.readerPluginName))
	c.jobWriter, err = c.initWriterJob(collector, readerConfig, writerConfig)
	if err != nil {
		return
	}
	slog.Info(fmt.Sprintf("DataX jobContainer %v writer %v inited", c.jobID, c.writerPluginName))
	return
}
...
func (c *Container) prepare() (err error) {
	...
	slog.Info(fmt.Sprintf("DataX jobContainer %v reader %v prepared", c.jobID, c.readerPluginName))
	if err = c.prepareWriterJob(); err != nil {
		return err
	}
	slog.Info(fmt.Sprintf("DataX jobContainer %v writer %v prepared", c.jobID, c.writerPluginName))
	return
}
...
func (c *Container) split() (err error) {
	...
	slog.Info(fmt.Sprintf("DataX jobContainer %v reader %v split %v tasks", c.jobID, c.readerPluginName, taskNumber))
	...
	slog.Info(fmt.Sprintf("DataX jobContainer %v writer %v split %v tasks", c.jobID, c.readerPluginName, len(writerConfs)))
...
func (c *Container) post() (err error) {
	...
	slog.Info(fmt.Sprintf("DataX jobContainer %v reader %v posted", c.jobID, c.readerPluginName))
	if err = c.jobWriter.Post(c.ctx); err != nil {
		return err
	}
	slog.Info(fmt.Sprintf("DataX jobContainer %v writer %v posted", c.jobID, c.writerPluginName))
	return
}
...
func (c *Container) mergeTaskConfigs(readerConfs, writerConfs []*config.JSON) (taskConfigs []*config.JSON, err error) {
	...
	slog.Info("DataX jobContainer %v tansformer config is %v", c.jobID, transformConfs)
...
func (c *Container) adjustChannelNumber() error {
	...
	if isChannelLimit := c.Config().GetInt64OrDefaullt(coreconst.DataxJobSettingSpeedChannel, 1) > 0; isChannelLimit {
		...
		slog.Info(fmt.Sprintf("DataX jobContainer %v set Channel-Number to %v channels.", c.jobID, c.needChannelNumber))
		return nil
	}
	...
	if isByteLimit := c.Config().GetInt64OrDefaullt(coreconst.DataxJobSettingSpeedByte, 0) > 0; isByteLimit {
		...
		slog.Info(fmt.Sprintf("DataX jobContainer %v set Max-Byte-Speed to %v bytes", c.jobID, globalLimitedByteSpeed))
	}
	...
	if isRecordLimit := c.Config().GetInt64OrDefaullt(coreconst.DataxJobSettingSpeedRecord, 0) > 0; isRecordLimit {
		...
		slog.Info(fmt.Sprintf("DataX jobContainer %v  set Max-Record-Speed to %v records", c.jobID, globalLimitedRecordSpeed))
	}
