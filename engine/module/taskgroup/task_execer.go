...
import (
	...
	"github.com/as-tool/as-etl-engine/engine/module/taskgroup/runner"
	"github.com/as-tool/as-etl-engine/engine/module/transport/channel"
	"github.com/as-tool/as-etl-engine/engine/module/transport/exchange"
...
func (t *taskExecer) Start() {
	...
	slog.Debug(fmt.Sprintf("taskExecer %v start to run writer", t.key))
	t.wg.Add(1)
	...
	go func() {
		...
		if err := t.writerRunner.Run(ctx); err != nil {
			slog.Error(fmt.Sprintf("writer task(%v) fail, err: %v", t.Key(), err))
			t.errors <- err
		} else {
			t.errors <- nil
		}
	...
	slog.Debug(fmt.Sprintf("taskExecer %v start to run reader", t.key))
	...
	go func() {
		...
		if err := t.readerRunner.Run(ctx); err != nil {
			slog.Error(fmt.Sprintf("reader task(%v) fail, err: %v", t.Key(), err))
			t.errors <- err
		} else {
			t.errors <- nil
		}
...
func (t *taskExecer) Do() (err error) {
	slog.Debug(fmt.Sprintf("taskExecer %v start to do", t.key))
	defer func() {
		t.attemptCount.Inc()
		slog.Debug(fmt.Sprintf("taskExecer %v end to do", t.key))
	}()
	// Execute Read-Write Runner - Executes the runner responsible for both reading and writing operations
	t.Start()
	slog.Debug(fmt.Sprintf("taskExecer %v do wait runner stop", t.key))
	cnt := 0
	for {
		select {
		case err := <-t.errors:
...
func (t *taskExecer) Shutdown() {
	slog.Debug(fmt.Sprintf("taskExecer %v starts to shutdown", t.key))
	defer slog.Debug(fmt.Sprintf("taskExecer %v ends to shutdown", t.key))
	t.wg.Add(1)
	go func() {
		t.wg.Done()
		for {
	...
	slog.Debug(fmt.Sprintf("taskExecer %v shutdown wait runner stop", t.key))
	t.wg.Wait()

	slog.Debug(fmt.Sprintf("taskExecer %v shutdown reader", t.key))
...
Loop:
	...
	slog.Debug(fmt.Sprintf("taskExecer %v shutdown writer", t.key))
