...
func NewContainer(ctx context.Context, conf *config.JSON) (c *Container, err error) {
	...
	slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v) reportInterval: %v retryInterval: %v retryMaxCount: %v config: %v",
...
func (c *Container) Start() (err error) {
	slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v)  start", c.jobID, c.taskGroupID))
	defer slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v)  end", c.jobID, c.taskGroupID))
	...
	slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v) manager config", c.jobID, c.taskGroupID))
	...
	slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v) start tasks", c.jobID, c.taskGroupID))
	...
	slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v) manage tasks", c.jobID, c.taskGroupID))
...
QueueLoop:
	...
	slog.Info(fmt.Sprintf("datax job(%v) taskgruop(%v) wait tasks end", c.jobID, c.taskGroupID))
...
func (c *Container) startTaskExecer(te *taskExecer) (err error) {
	slog.Debug(fmt.Sprintf("datax job(%v) taskgruop(%v) task(%v) push", c.jobID, c.taskGroupID, te.Key()))
	c.wg.Add(1)
	...
	go func(te *taskExecer) {
		slog.Debug(fmt.Sprintf("datax job(%v) taskgruop(%v) task(%v) start", c.jobID, c.taskGroupID, te.Key()))
		defer func() {
			slog.Debug(fmt.Sprintf("datax job(%v) taskgruop(%v) task(%v) end", c.jobID, c.taskGroupID, te.Key()))
			c.wg.Done()
		}()
		...
		for {
			...
			case te.Err = <-errChan:
				...
				if te.Err != nil && te.WriterSuportFailOverport() && te.AttemptCount() <= c.retryMaxCount {
					slog.Debug(fmt.Sprintf("datax job(%v) taskgruop(%v) task(%v) shutdown and retry. attemptCount: %v err: %v",
