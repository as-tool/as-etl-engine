...
func (w *Writer) Run(ctx context.Context) (err error) {
	defer func() {
		slog.Debug(fmt.Sprintf("datax writer runner %v starts to destroy", w.describe))
		if destroyErr := w.task.Destroy(ctx); destroyErr != nil {
			slog.Error(fmt.Sprintf("task destroy fail, err: %v", destroyErr))
		}
	}()
	slog.Debug(fmt.Sprintf("datax writer runner %v starts to init", w.describe))
	if err = w.task.Init(ctx); err != nil {
		return err
	}

	slog.Debug(fmt.Sprintf("datax writer runner %v starts to prepare", w.describe))
	if err = w.task.Prepare(ctx); err != nil {
		return err
	}

	slog.Debug(fmt.Sprintf("datax writer runner %v starts to StartWrite", w.describe))
	if err = w.task.StartWrite(ctx, w.receiver); err != nil {
		return err
	}

	slog.Debug(fmt.Sprintf("datax writer runner %v starts to post", w.describe))
