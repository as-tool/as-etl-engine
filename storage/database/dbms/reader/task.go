...
func StartRead(ctx context.Context, reader BatchReader, sender plugin.RecordSender) (err error) {
	...
	slog.Info(fmt.Sprintf("jobid %v taskgroupid %v taskid %v startRead begin", reader.JobID(), reader.TaskGroupID(), reader.TaskID()))
	defer func() {
		sender.Terminate()
		slog.Info(fmt.Sprintf("jobid %v taskgroupid %v taskid %v startRead end", reader.JobID(), reader.TaskGroupID(), reader.TaskID()))
	}()
