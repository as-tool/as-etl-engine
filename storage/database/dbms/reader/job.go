...
func (j *Job) split(ctx context.Context, number int, fetcher splitRangeFetcher) (configs []*config.JSON, err error) {
	...
	slog.Debug(fmt.Sprintf("jobID: %v start to split", j.JobID()))
	...
	slog.Debug(fmt.Sprintf("jobID: %v fetch split table end", j.JobID()))
	...
	slog.Debug(fmt.Sprintf("jobID: %v split fetchMin = %v", j.JobID(), minColumn))
	var maxColumn element.Column
	if maxColumn, err = fetcher.fetchMax(ctx, splitTable); err != nil {
		err = errors.Wrapf(err, "fetchMax fail")
		return
	}
	slog.Debug(fmt.Sprintf("jobID: %v split fetchMax = %v", j.JobID(), maxColumn))
